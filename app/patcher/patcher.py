...
class RomPatcher(object):
    ...
    def download_rom(self, rom_url):
        if not re.search(self.archive_pattern, rom_url, re.IGNORECASE) and not re.search(
            self.patch_pattern, rom_url, re.IGNORECASE
        ):
    ...
    def patch_rom(self, patch_file, source_ext):
        ...
        patch_command = [self.flips, "--apply", patch_file, self.source_rom_path, self.new_rom_path]
